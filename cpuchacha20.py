...
def chacha20_encrypt(file_data, key, nonce):
    """Performs ChaCha20 encryption and measures CPU cycles."""
    cipher = Cipher(algorithms.ChaCha20(key, nonce), mode=None, backend=default_backend())
    encryptor = cipher.encryptor()

    start_time = time.perf_counter_ns()  # Start timing
    cipher_text = encryptor.update(file_data)  # Encrypt data
...
if __name__ == "__main__":
    key = os.urandom(32)  # Generate a 256-bit ChaCha20 key
    ...
    if file_data:
        ...
        print(f"\nEncrypting file of size {file_size / (1024 * 1024):.2f} MB using ChaCha20...\n")
        ...
        for i in range(num_runs):
            cipher_text, cpu_cycles = chacha20_encrypt(file_data, key, nonce)
        ...
        print(f"\n✅ Average CPU Cycles for ChaCha20 encryption: {avg_cycles / 1e6:.2f} million cycles")
